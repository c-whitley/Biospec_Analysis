...
if __name__ == "__main__":
    ...
    with shelve.open(filename) as shelf:
    	...
    	mean_ref = shelf["mean_ref"]

    del shelf


    data.loc[:,data.columns] = savgol_filter(data.values, polyorder = 3, window_length = 11)

    data = data.apply(lambda row: Kohler(data.columns[::-1], row, mean_ref) 
                                          , raw = True, axis = 1)
    ...
    with shelve.open(filename) as shelf:
    	...
    	shelf["corrected"] = input_.apply(lambda row: Kohler(np.flip(input_.columns), row, mean_ref) 
                                          , raw = True, axis = 1)
    	shelf["mean_ref"] = mean_ref
